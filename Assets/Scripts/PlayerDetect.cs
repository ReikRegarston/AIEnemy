...
{
    public float radius = 20f, attack = 3f;
    public int damage = 5;
    ...
    private void DetecteAttack()
    {
        Collider[] hitCollider = Physics.OverlapSphere(transform.position, attack);
    ...
    IEnumerator SetDamage(Collider player)
    {
        while (true) 
        {
            player.GetComponent<PlayerHelth>().TakeDamage(damage);
